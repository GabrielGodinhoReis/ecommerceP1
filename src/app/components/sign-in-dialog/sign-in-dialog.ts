...
@Component({
  ...
  template: `
    <div class="p-6 max-w-[400px] flex flex-col bg-gray-900 text-white rounded-xl">
      <div class="flex justify-between items-center mb-4">
        <div>
          <h2 class="text-2xl font-bold">{{ isSignUp() ? 'Criar Conta' : 'Entrar' }}</h2>
          <p class="text-sm text-gray-400">
            {{ isSignUp() ? 'Preencha seus dados para se cadastrar' : 'Acesse sua conta para continuar' }}
          </p>
        </div>
      ...
      <form [formGroup]="authForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-3 mt-2">
        @if (isSignUp()) {
        ...
        <button mat-raised-button color="primary" type="submit" class="w-full py-3 !bg-purple-600 mt-2">
      ...
      <div class="mt-4 text-center text-sm text-gray-400">
        @if (isSignUp()) {
          Já tem uma conta?
          <button type="button" (click)="toggleMode()" class="text-purple-400 font-bold hover:underline bg-transparent border-0 cursor-pointer">
            Fazer Login
          </button>
        } @else {
          Não tem uma conta?
...
export default class SignInDialog {
  ...
  data = inject<{ checkout?: boolean; mode?: 'signin' | 'signup' }>(MAT_DIALOG_DATA, { optional: true });
